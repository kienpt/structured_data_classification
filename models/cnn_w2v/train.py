...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        cnn = TextCNN(
            sequence_length=x_train.shape[1]/FLAGS.embedding_dim,
        ...
        def dev_step(x_batch, y_batch, writer=None):
            ...
            step, summaries, loss, accuracy, predictions, real_y = sess.run(
                [global_step, dev_summary_op, cnn.loss, cnn.accuracy, cnn.predictions, cnn.real_y],
                feed_dict)

            diff_counter = collections.Counter(predictions - real_y)
            sum_counter = collections.Counter(predictions + real_y)
            true_positive = sum_counter[0] #predict = [0], real = [0] 
            true_negative = sum_counter[2] #predict = [1], real = [1]
            ...
            time_str = datetime.datetime.now().isoformat()
            print("{}: step {}, loss {:g}, acc {:g}, prec: {:g}, recall: {:g}, f1_measure: {:g}".format(time_str, step, loss, accuracy, precision, recall, f1_measure ))
            if writer:
